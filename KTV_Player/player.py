...
class LrcTimer(QTimer):
    ...
    def setLrc(self, lrc):
        self.lrcIn = lrc
        if self.lrcIn.__len__() > 0:
            self.lrc[3] = self.lrcIn[0][1]
        if self.lrcIn.__len__() > 1:
            self.lrc[4] = self.lrcIn[1][1]
        if self.lrcIn.__len__() > 2:
    ...
    def updateLrc(self):
        ...
        for i in range(self.lrcIn.__len__()):
            if t == self.startTime + self.lrcIn[i][0]:
                self.nowTime = t // 10
                if not (i + 1 >= self.lrcIn.__len__()):
                ...
                self.lrc[3] = '' if i + 1 >= self.lrcIn.__len__() else self.lrcIn[i + 1][1]
                self.lrc[4] = '' if i + 2 >= self.lrcIn.__len__() else self.lrcIn[i + 2][1]
                self.lrc[5] = '' if i + 3 >= self.lrcIn.__len__() else self.lrcIn[i + 3][1]
...
class MyHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path.startswith('/pause'):
            ...
            self.end_headers()
            player.server.pause.emit()

        elif self.path.startswith('/stop'):
            self.send_response(200)
            self.end_headers()
            player.server.stop.emit()

        elif self.path.startswith('/reload'):
            self.send_response(200)
            self.end_headers()
            player.server._reload.emit()

        elif self.path.startswith('/close'):
            self.send_response(200)
            self.end_headers()
            player.server._close.emit()
